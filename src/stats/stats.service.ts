...
import { PromoCode, PromoCodeStatus } from '../promo-codes/entities/promo-code.entity';
...
export class StatsService {
  ...
  async getCouponStats(): Promise<CouponStatsDto> {
    ...
    const activeCoupons = await this.promoCodeRepository
      ...
      .andWhere(
        '(promo_code.end_at IS NULL OR promo_code.end_at > :now)',
        { now },
      )
      .getCount();
    ...
    const expiredCoupons = await this.promoCodeRepository
      ...
      .where(
        ...
        { inactiveStatus: PromoCodeStatus.INACTIVE, activeStatus: PromoCodeStatus.ACTIVE, now },
      )
      .getCount();
